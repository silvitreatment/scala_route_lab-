...
**Что делает:**
- Хранит в PostgreSQL список тенантов (компаний-клиентов) и их API ключи
...
**Что делает:**
- Хранит в PostgreSQL: остановки, маршруты, рейсы, расписание
- Отвечает на вопросы:
...
|---|---|---|
| Tenant | PostgreSQL | транзакционные данные, редко меняются |
| Catalog | PostgreSQL | статика, нужны сложные JOIN запросы |
...
Статические данные (расписание):
  seed.sql → PostgreSQL (Catalog) → кэш в Redis (опционально)
...
Auth данные:
  PostgreSQL (Tenant) → кэш в Redis (TTL 300s)
